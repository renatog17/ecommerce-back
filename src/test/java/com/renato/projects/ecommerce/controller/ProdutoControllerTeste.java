...
public class ProdutoControllerTeste {
	...
	void deveListarUmProduto() throws Exception {
		...
		mockMvc.perform(get("/produtos")).andExpect(status().isOk()).andExpect(jsonPath("$.content.length()").value(1));
	}
	...
	void deveListarVariosProdutos() throws Exception {
		...
		mockMvc.perform(get("/produtos")).andExpect(status().isOk()).andExpect(jsonPath("$.content.length()").value(2));
	}

	@Test
	void deveRetornarListaVaziaQuandoNaoHouverProdutos() throws Exception {

		mockMvc.perform(get("/produtos")).andExpect(status().isOk()).andExpect(jsonPath("$.content.length()").value(0));
	}
	...
	void deveListarApenasProdutosAtivosDentreAtivoFalseETrue() throws Exception {
		...
		.andExpect(status().isOk()).andExpect(jsonPath("$.content.length()").value(2))
				.andExpect(jsonPath("$.content[0].nome").exists()).andExpect(jsonPath("$.content[1].nome").exists());
	}
	...
	@Test
	void deveRetornarListaVaziaQuandoNaoHouverProdutosCo() throws Exception {
	...
	void deveRetornarVazioQuandoCategoriaNaoPossuirProdutos() throws Exception {
		...
		mockMvc.perform(get("/produtos?categoriaId=1")).andExpect(status().isOk())
